...
func TestStatusIdleDefaults(t *testing.T) {
	...
	v := m.View()
	for _, want := range []string{"snappy", "step 0/40", "0/8k tok", "auto"} {
		if !contains(v, want) {
...
func TestStatusUpdatesOnProgress(t *testing.T) {
	m := sized(New(Config{Model: "snappy", MaxSteps: 40, MaxTokens: 8000}), tw, th)
	m = apply(m, progressMsg{Model: "snappy", Step: 3, MaxSteps: 40, Tokens: 1200, MaxTokens: 8000})
	v := m.View()
	for _, want := range []string{"snappy", "step 3/40", "1.2k/8k tok", "auto"} {
		if !contains(v, want) {
	...
	m2 := apply(m, progressMsg{Model: "snappy", Step: 4, MaxSteps: 40, Tokens: 1800, MaxTokens: 8000})
...
func TestStatusNarrowElidesGracefully(t *testing.T) {
	m := sized(New(Config{Model: "snappy", MaxSteps: 40, MaxTokens: 8000}), 30, 12)
